...
E.prototype.init_proxy_req = function(req, res){
    ...
    if (this.router.is_bypass_proxy(req)) {
        // 绕过代理
        // this.logger.notice("绕过代理: %s", req.url)
        return;
    } else {
        // this.logger.notice("初始化proxy req: %s", req.url)
    }
    // 这里统计所有请求
    // this.logger.warn('开始统计:' + req.url)
    this.usage_start(req);
